...
def _get_sequence(chrom, start, end, min_size=1000, crispred=None):
    ...
    if crispred is not None:
        ...
        for cc, cs, ce in crispred:
            ...
            if chrom == cc and min(end, ce) > max(cs, curr_start):
                ...
                if curr_start > cs:
                    seq += variables.hg38[chrom][curr_start:cs]
                curr_start = ce
        if curr_start < end:
            seq += variables.hg38[chrom][curr_start:end]
        #print(start, end, end-start, len(seq))

    else:
        seq = variables.hg38[chrom][start:end]
    ...
    if len(seq) < min_size:
        ...
        if start - ext_left < 0:
            ext_left = start
        elif diff - ext_left + end > len(variables.hg38[chrom]):
            ext_left = diff - (len(variables.hg38[chrom]) - end)
        curr_start = start - ext_left
        curr_end = end + diff - ext_left

        if curr_start < start:
            seq = variables.hg38[chrom][curr_start:start] + seq
        if curr_end > end:
            seq = seq + variables.hg38[chrom][end:curr_end]
    if start < 0 or end > len(variables.hg38[chrom]):
        return None
    return seq
...
def generate_data(pos_pairs, neg_pairs, annotations, binary=False, min_size=1000):
    ...
    chrom_sizes = {}
    with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'hg38.len'), 'r') as f:
        for r in f:
            tokens = r.strip().split()
